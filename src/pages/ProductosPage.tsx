...
export const ProductosPage = () => {
  ...
  const guardarProducto = (datos: Omit<Producto, "id">) => {
    if (productoEditar) {
      ...
      const productosActuales = ProductoService.listarProductos();

      const nuevoNumero =
        productosActuales.length > 0
          ? Math.max(
              ...productosActuales.map((p) => {
                const numero = parseInt(p.codigo.replace("COD-", ""));
      ...
      const nuevoCodigo = `COD-${String(nuevoNumero).padStart(3, "0")}`;
  ...
  const inversionTotal = productos.reduce(
    (total, prod) => total + prod.stock * prod.precioVenta,
    0,
  );
                  ...
                  productosFiltrados.map((producto) => (
                    <tr key={producto.id}>
                      ...
                      <td>
                        <span
                          className={`badge ${
                            producto.stock < 15 ? "bg-danger" : "bg-success"
                          }`}
                        >
                          {producto.stock}
                        </span>
